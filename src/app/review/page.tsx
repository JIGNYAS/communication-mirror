...
import { isAudioReviewReady } from "@/lib/review";
...
export default function ReviewPage() {
  ...
  if (!ready) return <AppShell active="review" eyebrow="Act II · The review" title="Opening the saved take…"><div className="empty-state"><Clock3 size={34} /><p>Checking this browser for your baseline.</p></div></AppShell>;

  if (!state.diagnostic.hasRecording) {
    return (
      <AppShell active="review" eyebrow="Act II · The review" title="There is nothing to judge yet.">
  ...
  return (
    <AppShell active="review" eyebrow="Act II · Isolated channels" title="Change one sense. Change what you notice." aside={<div className={reviewComplete ? "completion-pill done" : "completion-pill"}><Check size={16} /> {Object.values(state.review.completed).filter(Boolean).length}/3 PASSES</div>}>
...
function AudioTask({ goals, ratings, tonalityRating, onRate, onRateTonality }: AudioTaskProps) {
  return <><p className="eyebrow">Task · Listen only</p><h2>{goals.length ? "Did the voice embody your intention?" : "What feeling did the voice create?"}</h2><p>Listen once without taking notes. On the second pass, notice pace, volume, melody, pauses, and the feeling underneath the words.</p>{goals.length > 0 && <div className="rating-table">{goals.map((goal) => <div key={goal}><strong>{goal}</strong><div>{[1,2,3,4,5].map((rating) => <button aria-label={`${goal}: ${rating} out of 5`} className={ratings[goal] === rating ? "selected" : ""} key={rating} onClick={() => onRate(goal, rating)}>{rating}</button>)}</div></div>)}</div>}<div className="review-tonality"><div><strong>Tonality</strong><small>How clearly did a deliberate feeling come through? This is a self-rating, not a browser measurement.</small></div><div className="tonality-rating" aria-label="Tonality self-rating">{[1,2,3,4,5].map((rating) => <button aria-label={`Tonality: ${rating} out of 5`} className={tonalityRating === rating ? "selected" : ""} key={rating} onClick={() => onRateTonality(rating)}>{rating}</button>)}</div></div></>;
}
