  ...
  return (
    <AppShell active="more" tone="light" eyebrow="Tools and settings" title="Everything beyond the weekly loop.">
      ...
      <section className="data-workspace" aria-labelledby="data-heading">
        ...
        <input ref={importRef} className="sr-only" type="file" accept="application/json" aria-label="Restore Mirror notes backup" onChange={(event) => { const input = event.target; void restore(input.files?.[0]).finally(() => { input.value = ""; }); }} />
      </section>
    </AppShell>
  );
}
