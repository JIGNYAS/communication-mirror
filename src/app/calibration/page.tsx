...
export default function CalibrationPage() {
  ...
  const activePhase = phase ?? (saved.baselineRms ? "done" : "idle");
    ...
    <AppShell active="coach" tone="light" eyebrow="Sound check" title="Give volume a reference point." aside={<Link className="button secondary" href="/coach"><ArrowLeft size={16} /> Back to Insights</Link>}>
      <section className="calibration-stage">
        ...
        <div className="soundcheck panel">
          <div className={activePhase === "running" ? "calibration-mic live" : "calibration-mic"}><Mic2 size={40} /></div>
          <div aria-live="polite" aria-atomic="true">
            {activePhase === "running" ? <><strong className="calibration-count">{remaining}</strong><p>Keep speaking at Level 3</p></> : activePhase === "done" ? <><Check size={28} className="calibration-check" /><h2>Reference saved</h2><p>Level 3: {saved.baselineRms?.toFixed(4)} RMS<br />Level 5 target: {saved.targetRms?.toFixed(4)} RMS</p></> : <><Radio size={28} /><h2>Ready for sound check</h2><p>Your browser will ask for microphone permission.</p></>}
          </div>
          ...
          <button className="button primary full" disabled={activePhase === "running"} onClick={() => void startCalibration()}>{activePhase === "done" ? <RotateCcw size={17} /> : <Mic2 size={17} />}{activePhase === "done" ? "Calibrate again" : "Start 10-second calibration"}</button>
